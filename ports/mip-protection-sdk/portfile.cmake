...
if (VCPKG_TARGET_IS_WINDOWS)
    ...
    set(MATCH_RULE PATTERN "*.dll" PATTERN "*.lib" PATTERN "*.exp")
elseif(VCPKG_TARGET_IS_LINUX)
    if(EXISTS "/etc/os-release")
        file(READ "/etc/os-release" OS_RELEASE_CONTENT)
        string(REGEX MATCH "(^|\n)ID(_LIKE)?=[^\n]*rhel" IS_RHEL "${OS_RELEASE_CONTENT}")
        string(REGEX MATCH "VERSION_ID=\"?([0-9]+)" VERSION_MATCH "${OS_RELEASE_CONTENT}")
        set(VERSION_ID "${CMAKE_MATCH_1}")
        if(IS_RHEL)
            if(VERSION_ID STREQUAL "8")
                vcpkg_download_distfile(ARCHIVE
                    URLS "https://github.com/limarev/mipsdk-scraper/releases/download/mip-${VERSION}/mip_sdk_protection_rhel8_${VERSION}.tar.gz"
                    FILENAME mip_sdk_protection_rhel8_${VERSION}.tar.gz
                    SHA512 2f6c9c79697da592ab486751dd28cba63f63f2a14f8243ebcbf3411b0c214bd26c298dc99e5e224877203d7f2fc3a3adfea969deff128bf9c9b2c4c9d1b96443
                )
            elseif(VERSION_ID STREQUAL "9")
                message(FATAL_ERROR "RHEL 9 is not supported until the MIP SDK archive SHA512 is registered.")
            else()
                message(FATAL_ERROR "Unknown or unsupported version: ${VERSION_ID}")
            endif()
        else()
            vcpkg_download_distfile(ARCHIVE
        ...
        message(FATAL_ERROR "Cannot identify the Linux distribution because /etc/os-release is missing")
    endif()
